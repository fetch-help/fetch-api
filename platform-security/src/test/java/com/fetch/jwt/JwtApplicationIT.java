...
class JwtApplicationIT {
    ...
    public void testCreateToken() {
        ...
        ResponseEntity<String> token =
                this.restTemplate.postForEntity("http://localhost:" + port + "/api/v1/token/create?username=John", entity,
                        String.class, params);
    ...
    public void testValidateToken() {
        ...
        Map<String, String> params = new HashMap<String, String>();
        ResponseEntity<String> token =
                restTemplate.postForEntity("http://localhost:" + port + "/api/v1/token/create?username=John", entity,
                        String.class, params);

        ParameterizedTypeReference<HashMap<String, String>> responseType =
                new ParameterizedTypeReference<HashMap<String, String>>() {};

        RequestEntity<Void> request = RequestEntity.post(URI.create("http://localhost:" + port + "/api/v1/token/validate?token="+token.getBody()))
                .accept(MediaType.APPLICATION_JSON).build();

        ResponseEntity<HashMap<String, String>> response = restTemplate.exchange(request, responseType);

        System.out.println(response.toString());
        //{"iss":"fetch-api","exp":"1578925506","iat":"1578923706","username":"John"}
        System.out.println(response.toString());
        Assert.assertEquals(200, response.getStatusCode().value());
        Assert.assertTrue(response.getBody().containsKey("iss"));
        Assert.assertTrue(response.getBody().get("iss").equals("fetch-api"));
