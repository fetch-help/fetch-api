...
@SpringBootTest(classes = {
        MerchantApplication.class}, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@ActiveProfiles("test")
class MerchantApplicationIT {
    ...
    public void testCreateUserCheckLogin() {
        ...
        //{"iss":"fetch","exp":"1578925506","iat":"1578923706","username":"John"}
        System.out.println(response.toString());
        Assert.assertEquals(200, response.getStatusCode().value());
        Assert.assertTrue(response.getBody().containsKey("iss"));
        Assert.assertTrue(response.getBody().get("iss").equals("fetch"));
